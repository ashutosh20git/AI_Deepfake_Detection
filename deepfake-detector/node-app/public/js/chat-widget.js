...
export function renderChatWidget() {
  const shell = `
    <!-- Bubble -->
    <button id="chat-bubble-trigger" class="fixed bottom-6 right-6 w-14 h-14 rounded-full bg-zinc-900 border border-zinc-800 shadow-xl shadow-cyan-500/20 hover:border-cyan-500 flex items-center justify-center transition-all z-50 focus:outline-none focus:ring-2 focus:ring-cyan-500/20">
      <i data-lucide="message-circle" class="w-6 h-6 text-cyan-500"></i>
    </button>
    ...
    <div id="chat-panel" class="fixed bottom-6 right-6 w-[400px] h-[600px] max-h-[80vh] bg-zinc-900/95 backdrop-blur-xl border border-zinc-800 rounded-lg shadow-2xl flex-col hidden z-50 transform translate-y-4 opacity-0 transition-all duration-300">
      ...
      <div class="p-4 border-b border-zinc-800 flex justify-between items-center">
        ...
        <button id="close-chat-panel" class="text-zinc-400 hover:text-zinc-50 focus:outline-none focus:ring-2 focus:ring-cyan-500/20 rounded-md">
      ...
      <div id="chat-messages" class="flex-1 overflow-y-auto p-4 space-y-4 font-sans text-sm pb-10">
        <div class="py-16 text-center">
          ...
          <i data-lucide="inbox" class="h-12 w-12 text-zinc-700 mx-auto mb-4"></i>
          <h3 class="text-lg font-mono text-zinc-300">No session yet</h3>
          <p class="text-sm text-zinc-500 mt-2">Ask a question to start an assistant thread.</p>
        </div>
      </div>
      ...
      <div class="p-4 border-t border-zinc-800 bg-zinc-900 flex flex-col gap-2 rounded-b-lg shrink-0">
        <div class="flex gap-2">
            ...
            <button id="chat-send" class="w-10 h-10 rounded-md bg-zinc-800 text-zinc-400 flex items-center justify-center transition-colors focus:outline-none focus:ring-2 focus:ring-cyan-500/20 disabled:opacity-50 disabled:cursor-not-allowed">
...
function setupChatHandlers() {
  ...
  function appendUserMsg(text) {
    const emptyNode = msgContainer.querySelector('.py-16.text-center');
